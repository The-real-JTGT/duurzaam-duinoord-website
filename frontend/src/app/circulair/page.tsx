...
export default async function CirculairPage() {
  const puckData = await getPuckContent("/circulair");

  if (puckData) {
    return (
      <>
        <Render config={config} data={puckData} />
        <EditButton slug="circulair" />
      </>
    );
  }
    ...
    <EditButton slug="circulair" />
      ...
      <section className="relative w-full py-24 md:py-32 px-4 md:px-8 overflow-hidden">
        <div className="max-w-7xl mx-auto flex flex-col items-center text-center relative z-10">
          ...
          <h1 className="text-5xl md:text-7xl font-serif font-bold text-foreground mb-6 tracking-tight">
            <span className="italic text-primary">Circulaire</span> Economie
          </h1>
          <p className="text-lg md:text-xl text-foreground/80 max-w-2xl leading-relaxed">
            In een circulaire wijk bestaat afval niet meer, maar is het een grondstof voor iets nieuws.
            Minder consumeren, materialen delen en kapotte spullen repareren.
          </p>
        </div>
      ...
      <section className="w-full py-16 px-4 md:px-8">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { icon: "🍾", title: "Duinoord onder de Kurk", desc: "Succesvolle inzamelactie waarbij natuurkurk een tweede leven krijgt als isolatiemateriaal." },
              { icon: "🧱", title: "Tegelwipacties", desc: "Gewipte tegels krijgen een nieuwe bestemming of worden duurzaam afgevoerd." },
              { icon: "🎪", title: "Events & Inspiratie", desc: "Tijdens de Go Green Markt laten we hergebruik en reparatie in de praktijk zien." },
            ].map((item, i) => (
              <div key={i} className="bg-white rounded-3xl p-8 border border-border shadow-soft transition duration-500 hover:-translate-y-1 hover:shadow-soft-md group text-center">
                <div className="w-14 h-14 rounded-full bg-muted/30 flex items-center justify-center text-2xl mb-4 mx-auto group-hover:scale-110 transition duration-500">
                  {item.icon}
                </div>
      ...
      <section className="w-full py-16 px-4 md:px-8 bg-white/50">
        <div className="max-w-3xl mx-auto">
          ...
          {/* Hulp bij opruimen & hergebruik */}
          <div className="mb-10">
            <h3 className="text-2xl font-serif font-semibold mb-6 text-foreground">Hulp bij opruimen &amp; hergebruik</h3>
          ...
          {/* Samen delen & ruilen */}
          <div className="mb-10">
            <h3 className="text-2xl font-serif font-semibold mb-6 text-foreground">Samen delen &amp; ruilen</h3>
            <div className="bg-white rounded-3xl p-8 border border-border shadow-soft">
              <ul className="space-y-4">
                {[
          ...
          {/* Reparatie & vakmanschap */}
          <div>
            <h3 className="text-2xl font-serif font-semibold mb-6 text-foreground">Reparatie &amp; vakmanschap</h3>
            <div className="bg-white rounded-3xl p-8 border border-border shadow-soft">
              <ul className="space-y-4">
                {[
